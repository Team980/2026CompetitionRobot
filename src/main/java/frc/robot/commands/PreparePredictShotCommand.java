...
public class PreparePredictShotCommand extends Command {
    ...
    public static final InterpolatingTreeMap<Distance, Double> distanceToTimeMap = new InterpolatingTreeMap<>(
        (startValue, endValue, q) -> 
            InverseInterpolator.forDouble()
                .inverseInterpolate(startValue.in(Meters), endValue.in(Meters), q.in(Meters)),
        (startValue, endValue, t) ->
    ...
    static
    {
        distanceToTimeMap.put(Inches.of(52.0), 1.0);
        distanceToTimeMap.put(Inches.of(114.4),1.5);
        distanceToTimeMap.put(Inches.of(165.5),2.0);
    }
   ...
   // public static boolean isReadyToShoot;
    public PreparePredictShotCommand(Shooter shooter, Hood hood, Supplier<Pose2d> robotPoseSupplier) {
    ...
    //use lambda supplier of () ->
    public PreparePredictShotCommand(Shooter shooter, Hood hood, Supplier<Pose2d> robotPoseSupplier, ChassisSpeeds chassisSpeeds) {
        this.shooter = shooter;
        this.hood = hood;
        this.robotPoseSupplier = robotPoseSupplier;
    ...
    private Distance getPredictedDistanceToHub()
    {
        double time = distanceToTimeMap.get(getDistanceToHub());
        final Translation2d robotPosition = new Translation2d(robotPoseSupplier.get().getTranslation().getX()
        + time* chassisSpeeds.vxMetersPerSecond,
        robotPoseSupplier.get().getTranslation().getY()
        + time* chassisSpeeds.vyMetersPerSecond);
